...
# [2020.02.20 Ben]
RTMP_PUSH_ARGS=$(echo ${RTMP_PUSH_ARGS})
...
echo Creating application $STREAM_NAME
cat >>${NGINX_CONFIG_FILE} <<!EOF
...
# [2020.02.20 Ben]
    if [ "$PUSH" = "true" ]; then
        for PUSH_URL in $(echo ${RTMP_PUSH_URLS}); do
            echo "Pushing stream to ${PUSH_URL}/${STREAM_NAME} ${RTMP_PUSH_ARGS}"
            cat >>${NGINX_CONFIG_FILE} <<!EOF
            push ${PUSH_URL}/${STREAM_NAME} ${RTMP_PUSH_ARGS};
!EOF
            #PUSH=false
        done
    fi
